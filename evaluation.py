...
def Output_Result(doc4test):
    mp,mr,mf = evaluate_documents(doc4test,muc)
    bp,br,bf = evaluate_documents(doc4test,b_cubed)
    cp,cr,cf = evaluate_documents(doc4test,ceafe)
    metrics = {}
    metrics["muc"] = (mr,mp,mf)
    metrics["b3"] = (br,bp,bf)
    metrics["ceaf"] = (cr,cp,cf)

    average = float(mf+bf+cf)/3.0
