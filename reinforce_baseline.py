...
def main():
    ...
    for echo in range(nnargs["epoch"]):
        ...
        entropy_log_manager = Logger(Tensorboard+args.tb+"/acl2018/%d/entropy/manager"%echo, flush_secs=3)
        entropy_log_worker = Logger(Tensorboard+args.tb+"/acl2018/%d/entropy/worker"%echo, flush_secs=3)
            ...
            if rl["end"] == True:
                ...
                for data in tmp_data:
                    ...
                    for s,e in zip(rl["starts"],rl["ends"]):
                        ...
                        baseline = torch.sum(score*costs).data.cpu().numpy()[0]
